...
bot.on("message", async message => {
  ...
  if(message.author.id == "299495028756054016" || message.author.id == "430447525800181762" || message.author.id == "453970692266786816" || message.author.hasPermission("BAN_MEMBERS")){const botconfig = require("./botconfig.json");
const replace = require('replace-in-file');
const Discord = require("discord.js");
const translate = require('google-translate-api');
const async = require("async");
...
if(cmd === `${prefix}say`) {
 ...
 if(cmd === `${prefix}gayrate` && args == "") {
     if (message.author.id == "299495028756054016" || message.author.id == "430447525800181762" || message.author.id == "453970692266786816"  || message.author.id == "424953131386798080" && args == "") {
     let botembed = new Discord.RichEmbed()
 ...
 if(cmd === `${prefix}gayrate`) {
      if (args == "<@299495028756054016>" || args == "<@430447525800181762>" || args == "<@453970692266786816>" || args == "<@!424953131386798080>") {
     let botembed = new Discord.RichEmbed()
    .setTitle("Gayrate machine")
